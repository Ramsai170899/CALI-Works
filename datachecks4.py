...
def save_failed_records_to_csv(failed_checks):
    if failed_checks:
        ...
        if response:
            ...
            if output_path:
                with open(output_path, 'w', newline='') as csvfile:
                    ...
                    csv_writer.writerow(['Policy/CoI Number', 'Status', 'Failure Reason', 'Failed Conditions'])
                    for check in failed_checks:
                        csv_writer.writerow([check[0], check[1], check[2], ', '.join(check[3])])
                print(f"Failed records saved to {output_path}")

if __name__ == "__main__":
    input_dataset = load_input_file()
    if input_dataset is not None:
        failed_checks = perform_data_checks(input_dataset)
        print("\nData Check process Completed!\n")
        save_failed_records_to_csv(failed_checks)
